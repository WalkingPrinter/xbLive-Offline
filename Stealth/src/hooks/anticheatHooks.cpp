	...
	switch (latestTitle) 
	{
		case 0x415608C3: // Black ops 2
			...
			if (logMessageSize == 0x36) 
			{
				...
				//Utils::WriteFile(Utils::va(StrEnc("XBLIVE:\\Challenges\\logMessage_bo2_dirty_%i.bin"), (int)time(0)), logMessage, logMessageSize);
				...
				//Utils::WriteFile(Utils::va(StrEnc("XBLIVE:\\Challenges\\logMessage_bo2_clean_%i.bin)", (int)time(0)), logMessage, logMessageSize);
		...
		case 0x415608FC: // Ghosts
			...
			if (logMessageSize == Native::DecryptDWORD(0x8DFC5 /*0x3A*/))
			{
				...
				//Utils::WriteFile(Utils::va(StrEnc("XBLIVE:\\Challenges\\logMessage_ghosts_dirty_%i.bin"), (int)time(0)), logMessage, logMessageSize);
	...
	switch (challengeType)
	{
		case 0xC8: 
		{
			if (memoryHashInfo) 
			{
				...
				if (Native::Read4Byte(memoryHashInfo + 0x04) != 0x82100000 || Native::Read4Byte(memoryHashInfo + 0x08) != 0x711400 || Native::Read4Byte(memoryHashInfo + 0x10) != 0x2000) 
				{
