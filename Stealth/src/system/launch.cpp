...
HRESULT Launch::Initialize() {
	...
	if (!xbLive.bDevkit) {
		...
		if (Utils::FileExists(pLocation)) {
			LOG_DEV(StrEnc("Parsing launch path for boot xex from %s"), pLocation);
			...
			if (ini.IsGood()) {
				...
				if (str && strlen(str) > 3) {
					...
					strBootXex = sstr;
					LOG_DEV(StrEnc("Launch path acquired: %s"), strBootXex.c_str());
				} else {
...
HRESULT Launch::SetLiveBlock(bool bEnabled) {
	...
	if (!SetOptionValueByName((char*)StrEnc("liveblock"), &value)) {
		LOG_DEV(StrEnc("Failed toggling liveblock - %i"), value);
		return S_FAIL;
	}

	// set stronglive
	if (!SetOptionValueByName((char*)StrEnc("livestrong"), &value)) {
		LOG_DEV(StrEnc("Failed toggling livestrong - %i"), value);
		return S_FAIL;
	}

	// set xhttp
	if (!SetOptionValueByName((char*)StrEnc("xhttp"), &value)) {
		LOG_DEV(StrEnc("Failed toggling xhttp - %i"), value);
		return S_FAIL;
	}
...
BOOL Launch::IsLiveBlocked() {
	...
	if (!GetOptionValueByName((char*)StrEnc("liveblock"), &liveblockValue)) {
		LOG_DEV(StrEnc("Failed toggling liveblock - %i"), liveblockValue);
		return TRUE;
	}

	// get stronglive
	if (!GetOptionValueByName((char*)StrEnc("livestrong"), &liveStrongValue)) {
		LOG_DEV(StrEnc("Failed toggling livestrong - %i"), liveStrongValue);
		return TRUE;
	}
