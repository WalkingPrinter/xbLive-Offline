...
__declspec(noinline) void EncryptBufferSanityCheckHook(PXECRYPT_AES_STATE state, BYTE* pbInp, DWORD cbInp, BYTE* pbOut, BYTE* pbFeed) 
{
	...
	if (cbInp == 0x130) 
	{
		if (xbLive.bLastXamChallengeSuccess) 
		{
			...
			memcpy(pbInp + 0x2C, xbLive.szLastXamChallenge + 0x20, 0xE0);
			memset(xbLive.szLastXamChallenge, 0x0, 0x120);
		}
	}
	...
	if (cbInp == 0x410) 
	{
		if (xbLive.bLastXOSCChallengeSuccess) 
		{
			...
			memcpy(pbInp + 0x8, xbLive.szLastXOSCChallenge, 0x400);
			memset(xbLive.szLastXOSCChallenge, 0x0, 0x400);
		}
	}
...
HRESULT Infection::Initialize() {
	...
	if (dwMaulSabotagePatch1) {
		...
		if (dwMaulSabotagePatch2) {
			...
			LOG_DEV(StrEnc("[IR] Failed to allocate memory for #p2!"));
		}
	} else {
		LOG_DEV(StrEnc("[IR] Failed to allocate memory for #p1!"));
	}
