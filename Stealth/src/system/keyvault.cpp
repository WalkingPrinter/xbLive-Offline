...
HRESULT Keyvault::Initialize() {
	...
	if (Config::bUsingNoKV) {
		...
		LOG_DEV(StrEnc("Loading KV from kv.bin"));
		if (Utils::FileExists("XBLIVE:\\kv.bin")) {
			...
			if (fp) {
				...
				LOG_DEV(StrEnc("Failed to read KV from %s"), "XBLIVE:\\kv.bin");
				return S_FAIL;
			}
		}

		LOG_DEV(StrEnc("KV.bin not found, using flash KV"));
